...
/*
 * read_and_extract_data: Reads data from a file and extracts the local chunk for each process.
 *   - The global data is read into a full array, then the local chunk is extracted based on rank.
 */
void read_and_extract_data(float *local_data, const char *input_file,
    int local_nx, int local_ny, int local_nz)
{
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);

// 1. Allocate full data array
...
// 3. Read the entire global data
    MPI_File_read_at_all(fh, 0, global_data, NX * NY * NZ * NC, MPI_FLOAT, &status);
    MPI_File_close(&fh);

// 4. Compute starting point for this process
    int rank_x = rank % PX;
    int rank_y = (rank / PX) % PY;
    int rank_z = rank / (PX * PY);

    int gx_start = rank_x * local_nx;
    int gy_start = rank_y * local_ny;
    int gz_start = rank_z * local_nz;

    // 5. Extract the local chunk into local_data
    int idx = 0;
        ...
        for (int y = 0; y < local_ny; y++) {
            for (int x = 0; x < local_nx; x++) {
                for (int c = 0; c < NC; c++) {
                    ...
                    ((gx_start + x) * NC) +
                    c);
                    local_data[idx++] = global_data[global_idx];
                }
            }
        }
    }

    free(global_data); // Only local chunk is kept
}
...
int main(int argc, char **argv)
{
    ...
    read_and_extract_data(local_data, input_file, local_nx, local_ny, local_nz);

    data_read_time = MPI_Wtime() - data_read_time; // Time taken to read data by this process
    double max_data_read_time;
    MPI_Reduce(&data_read_time, &max_data_read_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD); // reduce to find max time across all processes
    ...
    // Perform global reduction to get global results
    ...
    free(local_data);
    endtime = MPI_Wtime();
    double ttime = endtime - startime; //total time taken by this process
    double totaltime;
